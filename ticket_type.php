...
$currency = EL()->options->general->get( 'currency' );
$currency = "";
...
<?php if ($el_message_cart == "") : ?>
	<div class="cart-ticket-info" data-enable-tax="<?php echo esc_attr($enable_tax) ?>" data-percent-tax="<?php echo esc_attr($percent_tax) ?>" data-seat-option="<?php echo esc_attr($seat_option) ?>" data-id-cal="<?php echo esc_attr($id_calendar) ?>" data-id-event="<?php echo esc_attr($id_event) ?>" data-setting="<?php echo esc_attr($data_settings) ?>" >
		<div class="error-empty-cart">
			<span class="empty-item-cart"><?php esc_html_e("Please Select Your Ticket", "eventlist") ?></span>
			<span class="error-empty"><?php esc_html_e("Please select seat", "eventlist") ?></span>
			<span class="error-duplicate"><?php esc_html_e("Error seat duplicate", "eventlist") ?></span>
		</div>
		...
		foreach ( $list_type_ticket as $ticket ) : 
			...
			?>
			<div class="item-ticket-type item-<?php echo esc_attr($ticket['ticket_id']) ?>" data-setup_seat="<?php echo esc_attr($ticket['setup_seat']) ?>" data-list-seat-rest="<?php echo esc_attr(json_encode($list_seat_rest)) ?>" >
				...
				<div class="quanty-ticket">
					...
					?>
					<span class="error error-min-num"><?php echo  esc_html__("Min number ticket is ", "eventlist") . $min_num ?></span>
					<div class="control">
						<?php $sold_out = true; if ( $max_num > 0 && $min_num <= $max_num ) : $flag_sold_out[] = "" ; $sold_out = false; ;?>
						<span class="minus" data-min-num="<?php esc_attr_e($min_num) ?>" data-price="<?php echo esc_attr( $price_calc ) ?>" data-title="<?php echo esc_attr( $ticket['name_ticket'] ) ?>" data-id-ticket="<?php echo esc_attr($ticket['ticket_id']) ?>"><i class="fas fa-minus"></i></span>
						<span class="qty qty-<?php echo esc_attr($ticket['ticket_id']) ?>">0</span>
						<span class="plus" data-mark="0" data-max-num="<?php esc_attr_e($max_num) ?>" data-price="<?php echo esc_attr( $price_calc ) ?>" data-title="<?php echo esc_attr( $ticket['name_ticket'] ) ?>" data-id-ticket="<?php echo esc_attr($ticket['ticket_id']) ?>"><i class="fas fa-plus"></i></span>
						<?php else : ?>
							<?php $flag_sold_out[] = 'sold_out'; ?>
							<span class="sold-out"><?php esc_html_e("Sold out", "eventlist") ?></span>
						<?php endif ?>
					</div>
					<span class="error error-max-num"><?php echo  esc_html__("Max number ticket is ", "eventlist") . $max_num ?></span>
					<?php if ( ! $sold_out ) : ?>
						<a id="btn-delete-item-cart" title="<?php esc_attr_e("Delete item to cart", "eventlist") ?>" data-min="<?php echo esc_attr($min_num) ?>" data-id="<?php echo esc_attr($ticket['ticket_id']) ?>" href="javascript: void(0)" class="btn-delete-item-cart"><i class="fas fa-times"></i></a>
					<?php endif ?>
				</div>
...
<?php else : //if message ?>
	<p class="error-item"><?php esc_html_e($el_message_cart) ?></p>
